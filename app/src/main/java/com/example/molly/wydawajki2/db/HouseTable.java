...
public class HouseTable
{
    ...
    public HouseTable(Context context)
    {
    ...
    public void insertRecord(Integer duserid, Double dusersum,String ddate)
    {
        ...
        sqLiteDatabase.insert(DBHelper.TABLE_DNAME,null,contentValues);
        Log.d("jest", "wpisano do house");
    }

    public Cursor getAllRecords()
    {
        Log.d("jest", "wypisujenmy");
        return sqLiteDatabase.rawQuery("select * from "+DBHelper.TABLE_DNAME,null);

    }

    public void deleteAllHouseExpenses()
    {
        sqLiteDatabase.delete(DBHelper.TABLE_DNAME,null,null);
    }

    public Cursor getSumAndDate(Integer userid)
    {
        Log.d("jest", "suma+data");
        return sqLiteDatabase.rawQuery("select "+DBHelper.COL_DUSERSUM+","+DBHelper.COL_DDATE+" from "+DBHelper.TABLE_DNAME
    ...
    {
        Log.d("jest", "suma");
        return sqLiteDatabase.rawQuery("select SUM("+DBHelper.COL_DUSERSUM+") "+" from "+DBHelper.TABLE_DNAME
                +" where "+DBHelper.COL_DUSERID+"="+userid,null);
